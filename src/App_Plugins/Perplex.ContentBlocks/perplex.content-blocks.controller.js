angular.module("umbraco").controller("Perplex.ContentBlocks.Controller", [
    "$scope", "$sce", "$element", "$q", "editorState", "eventsService", "$timeout", "formHelper",
    "Perplex.ContentBlocks.Api", "Perplex.ContentBlocks.CopyPaste.Service", "notificationsService", "Perplex.Util.Service",
    function ($scope, $sce, $rootElement, $q, editorState, eventsService, $timeout, formHelper, api, copyPasteService, notificationsService, utilService) {
        var vm = this;

        var constants = {
            headerCategoryId: "0a893d02-9166-437d-90e8-264524f410c0",
        ...
        var state = {
            ...
            metadata: {
                // ContentBlockDefinitionId => IContentBlockUsage
                uses: {}
            },
        ...
        var fn = {
            init: function () {
                ...
                api.getWebsiteId(state.pageId).then(function (response) {
                    state.websiteId = response.data;                    
                    return api.getAllContentBlockUses(state.websiteId, state.culture)
                }).then(function (response) {
                    state.metadata.uses = response.data;
                });     
            ...
            picker: {
                init: function (callback, disabledSelector) {
                    ...
                    // Hidden categorieën niet tonen in de category selector
            ...
            header: {
                ...
                pick: function () {
                    function disabledSelector(category) {
                        return category.id !== constants.headerCategoryId;
                    }
            ...
            getUses: function (definition) {
                var use = state.metadata.uses[definition.Id];
                if (use == null) {
                    return 0;
                }

                return use.TotalUses;
            },
